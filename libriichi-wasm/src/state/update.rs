...
impl PlayerState {
    ...
    fn tsumo(&mut self, actor: u8, pai: Tile) -> Result<()> {
        ...
        if self.riichi_accepted[0] {
            ...
            // Sanma: nukidora (抜きドラ) is allowed even during riichi (Tenhou
            // rule: extracting North does not change the wait). This must be
            // evaluated before the early return below.
        ...
        // tile after a normal (non-riichi) draw. Legality during riichi is
        // computed separately inside the riichi branch above (a North draw
        // during riichi keeps the wait, like an ankan, so it is still allowed
        // there — NOT via move_tile/validate). Disallow when no rinshan tiles
        // are left to draw (kans_on_board doubles as the shared rinshan-slot
        // counter).
        self.last_cans.can_nukidora =
            self.tehai[tuz!(N)] > 0 && self.tiles_left > 0 && self.kans_on_board < 4;
