    ...
    def populate_buffer(self, file_list):
        ...
        for file in data:
            for game in file:
                ...
                game_size = len(obs)
                if game_size == 0 or len(at_kyoku) == 0:
                    # Issue #103 (open): some games have no actions for the
                    # chosen player (e.g. pure ryukyoku from another seat's
                    # actions). Skip cleanly instead of IndexError on at_kyoku[-1].
                    continue
